...
const currentWeather = (lat, lon, callback) => {
    ...
    request({ url, json: true}, (error, { body }) => { // body is deconstructed from response object
        if(error) {
            ...
            callback(
                undefined,
                body.current.weather_descriptions[0] + ". It is " + body.current.temperature +"°F out, and feels like " + body.current.feelslike + 
                "°F. Humidity is " + body.current.humidity + " and there is a " + body.current.precip + '% chance of rain.'
            )
        }
    })
} 
